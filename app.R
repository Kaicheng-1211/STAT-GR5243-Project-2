...
guideServer <- function(id) {
    moduleServer(id, function(input, output, session) {
        ...
        observeEvent(input$goto_upload, {
            bslib::nav_select(id = "main_nav", selected = "upload")
        })

        observeEvent(input$goto_cleaning, {
            bslib::nav_select(id = "main_nav", selected = "cleaning")
        })

        observeEvent(input$goto_feature, {
            bslib::nav_select(id = "main_nav", selected = "feature")
        })

        observeEvent(input$goto_eda, {
            bslib::nav_select(id = "main_nav", selected = "eda")
        })

        observeEvent(input$goto_distributions, {
            bslib::nav_select(id = "main_nav", selected = "distributions")
        })

        observeEvent(input$goto_diagnostics, {
            bslib::nav_select(id = "main_nav", selected = "diagnostics")
        })
    })
}
